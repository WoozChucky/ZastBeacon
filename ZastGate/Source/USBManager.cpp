...
void USBManager::writeData(const char *message) {

    memcpy(&buffer, &message, sizeof(message));

    cout<<"Data->"<<buffer<<"<-"<<endl; //just to see the data we want to write : ABC
    cout<<"Writing Data..."<<endl;
    result = libusb_bulk_transfer(device_handle, (2 | LIBUSB_ENDPOINT_OUT), buffer, sizeof(buffer), &bytesRead, 0);
...
int USBManager::readData(unsigned char **response) {

    result = libusb_bulk_transfer(device_handle, (1 | LIBUSB_ENDPOINT_IN), buffer, sizeof(buffer), &bytesRead, 0);
    if(result == 0 && bytesRead == 8) { //we read the bytes successfully
        cout << "Read Successful!" << endl;

        memcpy(&response, &buffer, sizeof(buffer)); // copy buffer to command, to be sent to gate controller
